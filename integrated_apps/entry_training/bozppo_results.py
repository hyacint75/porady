...
class TrainingResultsMixin:
    def evaluate_test(self):
        ...
        if self._has_result_today() and not messagebox.askyesno(
            "Duplicitní záznam",
            "Stejný zaměstnanec ze stejného oddělení už má dnes uložené školení. Chcete uložit další záznam?",
        ):
            return
        ...
        if passed:
            messagebox.showinfo(
                "Test splněn",
                "Vstupní školení bylo úspěšně dokončeno. Záznam i potvrzení o absolvování byly uloženy.",
            )
        else:
    ...
    def _validate_identity(self):
        ...
        if not self.worker_name.get().strip():
            missing.append("jméno zaměstnance")
        if not self.company_name.get().strip():
            missing.append("oddělení / pracoviště")
    ...
    def _build_summary(self, correct, percent, result):
        lines = [
            f"Zaměstnanec: {self.worker_name.get().strip()}",
            f"Oddělení / pracoviště: {self.company_name.get().strip()}",
    ...
    def export_results_excel(self):
        ...
        headers = ["Datum", "Zaměstnanec", "Oddělení", "Školitel", "Pokus", "Skóre", "%", "Výsledek", "Platnost"]
        data = [
            [
                row["datum"],
                row["pracovnik"],
